...
/////////////////////////////////FILE HANDLING////////////////////////////
void save_file(const char *variable, float val){
     FILE *f = fopen("saved_vals.txt", "a"); //a used to write into the code without deleting prevoius data
...
void clear_file(void){
     FILE *f  = fopen("saved_vals.txt", "w"); //notice how this is a w instead of an a?
     if (!f) {
        printf("Error opening file.\n");
        return;
    } 

    printf("Deleted save file \n");
    fclose(f);
...
void menu_item_1(void) {
    printf("\n----------- Buck converter -----------\n");
    printf("\nSelect the wanted unknowns (inputs: 1,2,3)"
           "\t\t\t\t\t\t\n"
           "\t1. Find K\t\t\n"
           "\t2. Find Vi/Vo\t\t\n"
           "\t3. Back to main menu\t\t\t\t\n"
           "\t\t\t\t\t\t\n");
    printf("-----------(ALL UNITS ARE IN SI BASE UNITS)------------\n");
...
void menu_item_2(void) {
     printf("\n----------- FILE HANDLING -----------\n");
    printf("\nSelect the next action: (inputs: 1,2,3)"
           "\t\t\t\t\t\t\n"
           "\t1. Load file\t\t\n"
           "\t2. Clear file\t\t\n"
           "\t3. Back to main menu\t\t\t\t\n"
    ...
    void select_menu_item(int input); {
    switch (input) {
        case 1: menu9_item_load(); break; //Load file
        case 2: menu9_item_clear(); break; //Clear file
        case 3: printf("Back to main menu\n"); go_back_to_main();
        default: printf("Invalid selection. Exiting...\n"); exit(1);
    }
}
...
/////////////////////////////////////////////////////////////////////
//////////////////////////////////Buck menu 2 (FINDING K)////////////////////////
//////////////////////////////////////////////////////////////////////

void menu2_item_1(void) { //The first menu for finding K
    printf("\n----------- Buck converter -----------\n");
    printf("\nSelect the given variables (inputs: 1,2,3,4,5)"
           "\t\t\t\t\t\t\n"
           "\t1. Io\t\t\n"
           "\t2. Po/Pi, constant Vo\t\t\n"
           "\t3. Delta IL, IL peak\t\t\t\t\n"
   ...
   printf("\nInput Vo\n");
    ...
    save_file("Ts",ts);
        ...
        if (IO>IOB_MAX){ //CCM
           printf("\n----------- Buck converter -----------\n");
    printf("\nConver operates in CCM\n"
            "\nSelect the given variables: (inputs: 1,2,3,4)\n"
           "\t\t\t\t\t\t\n"
           "\t1. delta i\t\t\n"
           "\t2. Co\t\t\n"
           "\t3. Desired Vi\t\t\n"
   ...
   printf("\nVo constant given as %f\n", vo);
    ...
    printf("IOBMAX = %f", IOB_MAX);
        ...
        if (IO>IOB_MAX){
           printf("\n----------- Buck converter -----------\n");
    printf("\nConver operates in CCM\n"
            "\nSelect the given variables: (inputs: 1,2,3)\n"
           "\t\t\t\t\t\t\n"
           "\t1. delta i\t\t\n"
           "\t2. Co\t\t\n"
           "\t3. Desired Vi\t\t\n"
...
void menu7_item_Vi(float IO, float K) {
    ...
    if (IO>IOB_MAX){
           printf("\n----------- Buck converter -----------\n");
    printf("\nConver operates in CCM\n"
            "\nSelect the desired path: (inputs: 1,2)\n"
           "\t\t\t\t\t\t\n"
           "\t1. Vo\t\t\n"
           "\t2. Go back to main menu\t\t\t\t\n"
           "\t\t\t\t\t\t\n");
    ...
    switch (input) {
        ...
        printf("\n----------- Buck converter -----------\n");
    printf("\nConver operates in DCM\n"
            "\nSelect desired path: (inputs: 1,2)\n"
           "\t\t\t\t\t\t\n"
           "\t1. Vo\t\t\n"
           "\t2. Go back to main menu\t\t\t\t\n"
           "\t\t\t\t\t\t\n");
...
void menu3_item_3(void) {
    ...
    save_file("IO",IO);
    save_file("Delta IL",delta_IL);
    save_file("IL peak",IL_peak);
    //Ask for Vi/Vo constant
    printf("\n----------- Buck converter -----------\n");
    printf("\nWhich constant is given?"
           "\t\t\t\t\t\t\n"
...
void menu4_item_1(float IO){ //Given Ii
 printf("Enter Ii: \n");
    ...
    if (K<0 ||K>1){
         printf("\nK was out of the expected range of 0-1. Make sure the inputs are correct. ");
    }
    save_file("Ii",Ii);
    save_file("K",K);
...
void menu6_item_1(void) { //The main menu of finding vo/vo
    printf("\n----------- Buck converter -----------\n");
    printf("\nSelect the given variables (inputs: 1,2,3,4,5,6)"
           "\t\t\t\t\t\t\n"
           "\t1. K, Ii\t\t\n"
           "\t2. K, Constant Vo, RL\t\t\n"
